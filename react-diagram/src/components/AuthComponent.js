...
const AuthComponent = ({setIsAuthModalOpen}) => {
            ...
            try {
                axios.post('http://localhost:5000/auth/register', formData)
                    .then(response => {
                        ...
                        setAuthState('emailVerification');
                        setVerificationCode(response.data.user.verificationCode);
                    })
                    .catch(error => {
        ...
        const handleVerificationSubmit = async (e) => {
            ...
            const userEnteredCode = e.target[0].value;
        ...
        const handlePasswordRecoveryRequest = async (e) => {
            e.preventDefault();
            try {
                axios.post('http://localhost:5000/auth/request-password-reset', {email})
                    .then(response => {
                        console.log(response.data);
                        setRecoveryStep(2);
                        setVerificationCode(response.data.resetCode);
                    })
                    .catch(error => {
        ...
        const handlePasswordReset = async (e) => {
            e.preventDefault();
            console.log(verificationCode, e.target[1].value)
            if(verificationCode.toString() === e.target[1].value) {
                try {
                    axios.post('http://localhost:5000/auth/reset-password', {email})
                        .then(response => {
                            console.log(response.data);
                            setAuthState('login');
                        })
                ...
                case 'passwordRecovery':
                    return (
                        <div className={"passwordRecoveryBox"}>
                            ...
                            <form onSubmit={recoveryStep === 1 ? handlePasswordRecoveryRequest : handlePasswordReset}>
                                <input type="email" placeholder="E-mail*" required
                                       value={email}
                                       onChange={(e) => setEmail(e.target.value)}
                                />
                                ...
                                <button type="submit">{
                                    recoveryStep === 1 ? "Send code" : "Change password"
                                }</button>
                            </form>
                        </div>
                    );
                case 'emailVerification':
                    return (
                        <div className={"emailVerificationBox"}>
                            ...
                            <p>Enter the code from the email to verify your account</p>
